...
std::vector<ServerConf> ServerConf::parseConfigFile(const std::string &configFile)
{
	...
	size_t clientMaxBodySize = 10485760;
	while (getline(file, line))
	{
		...
		else if (starts_with(line, "upload_store") && inLocationBlock)
		{
    		...
    		std::cout << "Parsed upload_store: " << uploadPath << std::endl;
		}
		...
		else if (starts_with(line, "client_max_body_size") && !inLocationBlock)
		{
    		...
    		try {
        		clientMaxBodySize = stoi(sizeStr) * multiplier;
        		std::cout << "Parsed client_max_body_size: " << clientMaxBodySize << " bytes" << std::endl;
    		}
    		catch (...) {
